 ...
 *
 * At runtime the same predicates resolve against `context`, and only the
...
const toolSet = createToolSet<typeof tools, UIMessage, Context>({ tools })
  ...
  .deactivateWhen('list_orders', ({ context }) => context?.isAdmin === false) // active by default
  .activateWhen('refund', ({ context }) => context?.isAdmin === true); // inactive by default
...
// At runtime, the conditional tools flip based on `context`:

const asUser = toolSet.inferTools({ context: { isAdmin: false } });
console.log(`non-admin → activeTools: [${asUser.activeTools.join(', ')}]`);
//   search, but list_orders is deactivated and refund stays off

const asAdmin = toolSet.inferTools({ context: { isAdmin: true } });
